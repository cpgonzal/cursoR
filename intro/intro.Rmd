...
y<-c(1:5)
?seq 
# los argumentos con = significa que hay que escribir el nombre
# los argumentos sin = significa que hay que ponerlos en el mismo orden
z<-seq(1,10,by=2)
w<-rep(1:5,times=2)
w<-rep(1:5,each=3)
```
...
x+z
x*z
# se pueden sumar vectores de distinta longitud pero se 
# produce un efecto cíclico 
x+w
...
x['segundo']  # accede al elemento "el segundo"
x[segundo]    
```
...
# observar que se emplea = para asignar los nombres 
# a los objetos y no el operador <-
attributes(l)
names(l)  
is.list(l)
...
names(data.geo.municipios)[4]<-"Perimetro.municipal"
```
...
``` {r chart01, echo=FALSE, message=FALSE,eval=TRUE, out.extra='style="display:block; margin:auto;"'}
plot(Superficie~Perimetro.municipal,data=data.geo.municipios,
xlab="Perímetro del municipio (km)",
ylab="Superficie (km. cuadrados)",
main="Comparación del perímetro y la superficie de los municipios")
```
...
``` {r chart02a, echo=TRUE, message=FALSE,eval=FALSE}
...
``` {r chart02b, echo=FALSE, message=FALSE,eval=TRUE, out.extra='style="display:block; margin:auto;"'}
plot(Superficie~Altitud,data=data.geo.municipios)
```
...
``` {r chart03a, echo=TRUE, message=FALSE,eval=FALSE}
...
``` {r chart03b, echo=FALSE, message=FALSE,eval=TRUE, out.extra='style="display:block; margin:auto;"'}
data.geo.municipios$Provincia<-as.factor(data.geo.municipios$Provincia)
plot.colors<-c("green","orange")

plot(Superficie~Perimetro.municipal,data=data.geo.municipios,col=plot.colors[data.geo.municipios$Provincia],pch=20)
legend("topleft",legend=levels(data.geo.municipios$Provincia),col=c("green","orange"),pch=rep(20,2))
```
---
...
``` {r chart04a, echo=TRUE, message=FALSE,eval=FALSE}
library(lattice)
...
``` {r chart04b, echo=FALSE, message=FALSE,eval=TRUE, out.extra='style="display:block; margin:auto;"'}
library(lattice)
xyplot(Superficie ~ Perimetro.municipal | Provincia, data=data.geo.municipios)
```
...
mystats(data.pob.municipios[data.pob.municipios$CodMunicipio %in% c(38013,38048,38901) & data.pob.municipios$Indicador=='Cifras absolutas' & data.pob.municipios$Anio=='2012',"Valor"])
```
